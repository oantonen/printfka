...
char			*pf_float_entier_count(intmax_t nb, int *chars)
{
	...
	*chars = i;
	...
	// *chars += (w > *chars && ISZERO) ? w - *chars - p - 1 : 0;
...
long double		pf_round_float(long double nb, int prec, float_cast	d1)
{
...
char			*pf_float_f_fraction(long double nb, float_cast	d1)
{
	long double		fraction;
	int				first_digit;
	int				i;
